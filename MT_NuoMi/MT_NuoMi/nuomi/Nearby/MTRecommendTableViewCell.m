...
#import "MTNearbyView.h"


@interface MTRecommendTableViewCell ()<UIGestureRecognizerDelegate>

@property (nonatomic, strong) UILongPressGestureRecognizer *pressRecognizer;

@end

@implementation MTRecommendTableViewCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier array:(NSArray *)array //形成cell
{
    ...
    if (self)
    {
        ...
        for (int i = 0; i < array.count; i++)
        {
            ...
            MTNearbyView *backView = [[MTNearbyView alloc] initWithFrame0:CGRectMake(i*[UIScreen mainScreen].bounds.size.width/3, 30, [UIScreen mainScreen].bounds.size.width/3, 170)
                                                                   name:act[@"name"]
