...
#import "MTImageWebViewController.h"
#import "MTLoginAndRegisterViewController.h"
...
#import "UINavigationBar+MTCategory.h"
...
@interface MTItemViewController ()<UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate, UIWebViewDelegate, MTItemGroupTableViewCellDelegate>
...
@property (nonatomic, strong) MTButtonMenuView *titleView;
...
@implementation MTItemViewController
...
{
    MTLoginAndRegisterViewController *denglu = [[MTLoginAndRegisterViewController alloc]initWithNibName:@"MTLoginAndRegisterViewController" bundle:nil];
...
-(void)initTableView
{
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, MTScreenWidth, MTScreenHeight-40) style:UITableViewStyleGrouped];
    ...
    UINib *nib0 = [UINib nibWithNibName:@"MTPriceTableViewCell" bundle:nil];
    //通过UINib对象注册相应的NIB文件
...
-(void)initScrollView
{
    ...
    self.scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, MTScreenWidth, MTScreenWidth*0.65)];
    ...
    //背景图片
    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, MTScreenWidth, MTScreenWidth*0.65)];
...
{
    //标题文字
    _titleView = [[MTButtonMenuView alloc] initWithFrame9:CGRectMake(0,MTScreenWidth*0.4-20, MTScreenWidth, MTScreenWidth*0.2) subtitle:@"已售1531" title:_itemModel[@"title"] long_title:_itemModel[@"long_title"]];
...
-(void)setFooter
{
    ...
    [self.mai addTarget:self action:@selector(registerButton:) forControlEvents:UIControlEventTouchUpInside];
    ...
    if (scrollView == self.tableView)
    {
        ...
        if (offsetY < 60)
        {
            if (self.led == NO)
            {
                self.led = YES;
                self.lab.text = @"";
                UIImage *home1008 = [UIImage imageNamed:@"home-10-7"];
                [self.fenxiang setBackgroundImage:home1008 forState:UIControlStateNormal];
                
                [self.closeBtn setImage:[UIImage imageNamed:@"brower_pre_0"] forState:UIControlStateNormal];
              
                self.NavView.layer.borderWidth = 0;//边框线
        ...
        }else
        {
            if (self.led == YES)
            {
                self.led = NO;
                self.lab.text = _itemModel[@"title"];
                UIImage *home1008 = [UIImage imageNamed:@"home-10-4"];
                [self.fenxiang setBackgroundImage:home1008 forState:UIControlStateNormal];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0)
    {
        MTPriceTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell0" forIndexPath:indexPath];
    ...
    }else
    {
        ...
        }else
        {
            ...
            MTItemGroupTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
            
            if(cell == nil)
            {
                cell = [[MTItemGroupTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier array:_likeArray];
            }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1)
    {
        ...
        if (indexPath.row == 2)
        {
            MTImageWebViewController *item = [[MTImageWebViewController alloc] init];
...
{
    MTShopTuanModel *shopM = _likeArray[index];
    ...
    NSURLSessionDataTask *dataTask = [self.session dataTaskWithRequest:request
                                                     completionHandler: ^(NSData *data, NSURLResponse *response, NSError *error)
                                      {
                                          if (error) {
                                              ...
                                              NSString *urlString = _itemModel[@"consumer_tips"];
//                                              urlString = [urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:urlString]];
                                              ...
                                              [_webView1 loadHTMLString:urlString baseURL:nil];
                                              
                                              dispatch_async(dispatch_get_main_queue(),^{
                                                  NSLog(@" 刷新成功4 ");
                                                  
                                                  [self getRecommendData];
                                                  [_imageView sd_setImageWithURL:[NSURL URLWithString:_itemModel[@"image"]] placeholderImage:[UIImage imageNamed:@"icon_nav_cart_normal"]];
